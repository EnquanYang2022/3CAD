...
class MVTec2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dtd_img_src = self.dtd_imgs_list[idx]
        ...
        if self.phase=='train':
            ...
            return {
                # 'img_origin': img_origin, 'img_aug': aug_image
...
if __name__ == '__main__':
    train_dataset = MVTec2D(data_path='./MvtecAD/',dtd_path='./dtd/images/',class_name='bottle',phase='train')
    kwargs = ({"num_workers": 8, "pin_memory": True} if torch.cuda.is_available() else {})
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=8, shuffle=False,**kwargs)
    num = 000
    for _,d in enumerate(train_loader):
        print(num)
        print(d['img_aug'].shape) #16,3,256,256
        num+=1
